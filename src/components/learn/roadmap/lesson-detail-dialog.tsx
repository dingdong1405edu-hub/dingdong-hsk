"use client";

import { toast } from "sonner";
import { Sparkles, Star } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
} from "@/components/ui/dialog";
import { cn } from "@/lib/utils";
import { SkillTile } from "./skill-tile";
import { SKILL_META, type CourseTheme, type RoadmapLessonDTO } from "@/lib/roadmap";

interface LessonDetailDialogProps {
  lesson: RoadmapLessonDTO | null;
  levelLabel: string;
  theme: CourseTheme;
  onOpenChange: (open: boolean) => void;
}

/**
 * Hộp thoại chi tiết một bài: chủ đề + 6 kỹ năng (từ vựng, ngữ pháp, nghe, nói,
 * đọc, viết). Nội dung từng kỹ năng sẽ được nối sau — hiện hiển thị trạng thái
 * "Sắp có" và báo toast khi bấm.
 */
export function LessonDetailDialog({ lesson, levelLabel, theme, onOpenChange }: LessonDetailDialogProps) {
  const open = lesson !== null;
  const publishedBySkill = new Map((lesson?.sections ?? []).map((s) => [s.skill, s.published]));
  const doneSkills = new Set(lesson?.skillsDone ?? []);

  function handleSkill(label: string, published: boolean) {
    if (published) {
      toast.info(`Đang mở phần "${label}"…`);
    } else {
      toast.info(`Phần "${label}" đang được biên soạn — sắp ra mắt! ✨`);
    }
  }

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md gap-0 overflow-hidden rounded-2xl p-0 [&>button]:text-white [&>button]:opacity-90 [&>button:hover]:opacity-100">
        {/* Header có màu theo khóa */}
        <div className={cn("relative bg-gradient-to-br p-5 text-white", theme.hero)}>
          <div className="relative z-10 flex items-center gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white/20 text-3xl backdrop-blur">
              {lesson?.icon ?? "📘"}
            </div>
            <DialogHeader className="space-y-0.5 text-left">
              <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-white/80">
                <span>{levelLabel}</span>
                <span className="h-1 w-1 rounded-full bg-white/60" />
                <span>Bài {lesson?.order}</span>
              </div>
              <DialogTitle className="text-xl font-extrabold leading-tight text-white">
                {lesson?.topic}
              </DialogTitle>
              <DialogDescription className="font-chinese text-sm text-white/85">
                {lesson?.topicZh}
              </DialogDescription>
            </DialogHeader>
          </div>
          <div className="relative z-10 mt-3 inline-flex items-center gap-1.5 rounded-full bg-white/15 px-2.5 py-1 text-xs font-semibold backdrop-blur">
            <Star className="h-3.5 w-3.5 fill-white" /> +{lesson?.xpReward ?? 20} XP khi hoàn thành
          </div>
          <div className="pointer-events-none absolute -right-3 -top-6 select-none font-chinese text-8xl leading-none text-white/10">
            {theme.char}
          </div>
        </div>

        {/* 6 kỹ năng */}
        <div className="p-5">
          {lesson?.description && (
            <p className="mb-4 text-sm leading-relaxed text-muted-foreground">{lesson.description}</p>
          )}
          <div className="mb-3 flex items-center gap-2 text-[13px] font-bold uppercase tracking-wide text-muted-foreground">
            <Sparkles className={cn("h-4 w-4", theme.accentText)} /> Các kỹ năng trong bài
          </div>
          <div className="grid gap-2.5">
            {SKILL_META.map((meta) => (
              <SkillTile
                key={meta.key}
                meta={meta}
                published={publishedBySkill.get(meta.key) ?? false}
                done={doneSkills.has(meta.key)}
                onClick={() => handleSkill(meta.label, publishedBySkill.get(meta.key) ?? false)}
              />
            ))}
          </div>
          <p className="mt-4 text-center text-[11px] text-muted-foreground">
            Hoàn thành cả 6 kỹ năng để chinh phục bài học và mở khoá bài tiếp theo.
          </p>
        </div>
      </DialogContent>
    </Dialog>
  );
}
