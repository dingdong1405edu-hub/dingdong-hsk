...
| Text-to-Speech | **Không dùng AI TTS** — admin tự tải MP3 thật cho bài nghe (Deepgram & Groq không có giọng TTS tiếng Trung). Thiếu MP3 → trình duyệt đọc bằng giọng `zh-CN` |
...
**Lý do chọn Deepgram** (STT): Deepgram Nova-3 nhận dạng tiếng Trung Mandarin (`zh-CN`) độ chính xác cao, gọi trực tiếp qua REST `/v1/listen` nên không cần thêm SDK; Groq Whisper làm dự phòng STT. **Lưu ý quan trọng về TTS:** cả Deepgram lẫn Groq KHÔNG có giọng TTS tiếng Trung, nên hệ thống KHÔNG tạo audio bằng AI — admin **tự tải file MP3 thật** lên cho bài nghe. Khi một bài chưa có MP3, người học vẫn nghe được nhờ giọng đọc `zh-CN` của trình duyệt (Web Speech). Phần admin bài nghe vẫn dùng Deepgram để "tạo transcript từ audio" và Groq để sinh câu hỏi + dịch/giải thích lời thoại.
...
DEEPGRAM_API_KEY=""
# (Không có biến TTS — admin tự tải MP3 thật; thiếu MP3 thì trình duyệt tự đọc zh-CN.)
...
- **Audio tiếng Trung (TTS)**: KHÔNG tạo bằng AI — admin tự tải MP3 thật (Deepgram/Groq không có giọng TTS tiếng Trung). Khi bài chưa có MP3, người học nghe bằng Web Speech `zh-CN` của trình duyệt (xem `use-listening-audio.ts`).
...
- Khi user paste token (Groq / Deepgram / Railway): **không** echo lại token; chỉ confirm "đã nhận" và lưu vào `.env` (hoặc `.env.local`).
...
- Tokens cần thiết: `GROQ_API_KEY`, `DEEPGRAM_API_KEY`, `RAILWAY_TOKEN`, `GITHUB_TOKEN`.
